...
public class UserController {
    ...
    public ResponseEntity<AppUser> getCurrentUser(@AuthenticationPrincipal Jwt jwt) {
        ...
        if (email == null) {
            // Attempt an alternative claim namespace if configured with rules/actions, e.g., "https://aiGym/email"
        ...
        AppUser user = userRepository.findByAuth0Id(auth0Id)
                .orElseGet(() -> {
                    AppUser newUser = new AppUser(jwt.getSubject(), jwt.getClaimAsString("email") != null ? jwt.getClaimAsString("email") : "temp-email@domain.com");
